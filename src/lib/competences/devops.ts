...
La sécurité de la chaîne logicielle fait partie de la compétence : versions d'images, dépendances, provenance, secrets et permissions du pipeline. Une sauvegarde n'est utile que si elle correspond à un état cohérent et peut devenir un point de restauration.`,
    en: `DevOps brings together the practices that make a change reproducible from the code repository through operation: continuous integration, testing, build, deployment, configuration, secrets, backups, monitoring, and rollback. The objective is not merely automating a command. The gap between the tested environment and the one actually serving users must be reduced, and sufficient evidence retained to understand a failure.
...
Software supply-chain security is part of the skill: image versions, dependencies, provenance, secrets, and pipeline permissions. A backup is useful only when it represents a consistent state and can become a restoration point.`,
  ...
  anecdotes: [
    {
      ...
      result: {
        fr: "La production a repris après environ trois heures et aucun retour arrière n'a été nécessaire. Le scénario restait néanmoins prêt, ce qui a permis de traiter la bascule comme une opération contrôlée plutôt que comme un pari sur la réussite du premier démarrage.",
        en: "Production resumed after roughly three hours and no rollback was required. The scenario was nevertheless ready, allowing the cutover to be treated as a controlled operation rather than a bet on the first startup succeeding.",
      },
    ...
    },
    {
      ...
      result: {
        ...
        en: "The project already has a reproducible chain through previews and control foundations before its future production release. The platform choice remains evaluated by its cost-to-service ratio rather than treated as permanent.",
      },
    ...
    },
    {
      ...
      content: {
        fr: `Le portfolio utilise plusieurs niveaux de tests : fonctions unitaires sous jsdom, composants dans un navigateur Playwright, régressions visuelles et parcours de bout en bout. GitHub Actions exécute les contrôles afin qu'une modification ne dépende pas uniquement de mon environnement local. Les secrets de Resend, reCAPTCHA et Google Tag Manager restent fournis par variables d'environnement. Les dépendances externes sont simulées dans une partie des tests, limite que je documente au lieu de confondre une suite verte avec une validation complète du déploiement.`,
        en: `The portfolio uses several testing levels: unit functions under jsdom, components in a Playwright browser, visual regressions, and end-to-end journeys. GitHub Actions runs checks so a change does not depend solely on my local environment. Resend, reCAPTCHA, and Google Tag Manager secrets remain provided through environment variables. External dependencies are simulated in part of the tests, a limitation I document instead of confusing a green suite with complete deployment validation.`,
      },
      result: {
        ...
        en: "More than 170 tests now protect the main behaviors and content relationships. This automation speeds up change while keeping real checks still required at external-service boundaries explicitly identified.",
      },
  ...
  evolution: {
    ...
    training: {
      fr: "Je poursuis Docker, les images minimales, les SBOM, les pipelines CI/CD et le monitoring. Mon futur outil de Business Intelligence auto-hébergé doit servir de terrain pour pratiquer les conteneurs, la persistance, les sauvegardes, les métriques et l'exploitation régulière d'un service.",
