...
I pay particular attention to integrations. When two systems exchange customers, products, orders, or emails, apparent availability does not prove that data is complete and consistent. I therefore verify formats, errors, recovery behavior, possible duplicates, and each system's responsibility.`,
  ...
  anecdotes: [
    {
      ...
      result: {
        ...
        en: "The B2B website and main integrations resumed operation after the migration. Custom routes remain localized in version-controlled modules, making their responsibility and evolution identifiable.",
      },
    ...
    },
    {
      ...
      content: {
        fr: `Pour CAP2vie, l'équipe a construit un backend JavaScript avec Express, PostgreSQL et Prisma. Nous avons défini les principales entités du questionnaire, stocké les réponses et associé les événements aux dimensions de trajectoire. En cours de projet, j'ai pris le lead sur l'interconnexion multi-écrans avec Socket.io : création d'une room identifiée et protégée par mot de passe, synchronisation de l'état nécessaire au second écran et maintien de la saisie lorsque l'affichage secondaire se déconnectait. Le prototype restait local et la reconnexion était encore manuelle, limites que nous avons documentées.`,
      ...
      result: {
        fr: "La démonstration multi-écrans a fonctionné et le développeur suivant a réussi à installer le projet. L'expérience m'a appris à raisonner sur l'état d'une connexion temps réel, ses interruptions et le périmètre de sécurité acceptable uniquement pour une preuve de concept contrôlée.",
    ...
    },
    {
      ...
      content: {
        fr: `Le formulaire du portfolio utilise une Server Action Next.js qui revalide les données avec Zod avant d'appeler Resend. J'ai ajouté plusieurs couches contre les soumissions indésirables : champ honeypot, contrôle du temps écoulé, limitation à cinq requêtes par heure et par adresse IP, puis reCAPTCHA v3 lorsqu'il est configuré. Le navigateur ne décide jamais seul de la validité. Les réponses d'erreur restent génériques pour l'utilisateur tandis que les tests simulent les dépendances externes afin de couvrir les embranchements sans envoyer de vrais courriels.`,
        en: `The portfolio contact form uses a Next.js Server Action that revalidates data with Zod before calling Resend. I added several layers against unwanted submissions: a honeypot field, elapsed-time check, a limit of five requests per hour per IP address, and reCAPTCHA v3 when configured. The browser never decides validity alone. Error responses remain generic for users, while tests simulate external dependencies to cover branches without sending real emails.`,
      },
      result: {
  ...
  selfCritique: {
    level: {
      fr: "Je situe mon niveau à intermédiaire. Je sais concevoir des modèles, des règles métier, des API et des intégrations dans plusieurs écosystèmes, puis les tester et les déployer. Mon expérience la plus profonde reste Odoo ; sur Symfony, l'architecture distribuée et les systèmes temps réel à grande échelle, je manque encore de projets récents menés jusqu'à une forte charge de production.",
    ...
    importance: {
      fr: "Le backend relie ma spécialisation ERP à mon profil fullstack. Il me permet de ne pas traiter Odoo comme une application isolée : commandes B2B, logistique, courriels, données analytiques et interfaces publiques dépendent de contrats serveur fiables.",
    ...
    advice: {
      fr: "Je conseille de commencer une API par son contrat, ses acteurs et ses erreurs avant d'écrire la route heureuse. Il faut savoir qui possède la donnée, quelles validations sont autoritaires, comment l'appel peut être rejoué et quelle trace permettra de diagnostiquer un échec. Les contrôles d'interface complètent cette protection, mais ne remplacent jamais ceux du serveur.",
